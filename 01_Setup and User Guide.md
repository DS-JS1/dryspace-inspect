...
> **What changed in v1.3.** Photos now upload to SharePoint by themselves; staff
> sign in once with their Dryspace Microsoft account; and the app tells you when a
> new version is available instead of updating silently. Parts D, E and F changed
> most.
...
3. Change the stage to *02 Field*, then tap **Share draft**.
4. Follow the five-step handover ritual in `03_Handover and Version Control Protocol`.

### Field workflow (site visit)

1. Import the draft from SharePoint, or tap **+ New inspection** for a walk-in job.
...
7. When finished, set the stage to *03 Technical* and hand over — **after checking
   the photos have uploaded.**
...
Back in signal — car, office, wifi — tap **Upload now** on the home screen. It
...
## Part G — Known limits of v1.3

- **No live cross-device sync** — by design, for offline reliability. The draft file
  plus the SharePoint protocol is the transfer mechanism.
